...
/**
 * Example Servlet that uses a popup UI to log users in using a 'login with
 * Google' button
 * 
...
public class LoginViaPopupServlet extends InjectableServlet {
	...
	// private static final String OPENID_2_0_SERVER =
	...
	private static final Logger logger = Logger
			.getLogger(LoginViaPopupServlet.class.getCanonicalName());
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		session.setAttribute(REALM,
				realm.toString().replaceFirst("http://www\\.", "http://*."));

		// Now set the return_to URL
		realm.append(PROJECT).append(REDIRECT_PATH);
	 ...
	 * an RP could use some technique to identify the user's preferred identity
	 * provider.
	 * 
	 * @param req
	...
	private void perOpCustomize(HttpServletRequest req) throws ServletException {
		...
		// TODO(breno): This assumes that the OP is 'Google Accounts' so it
		// performs
		// discovery at that location. In practice, this needs to be generalized
		// for
		// other OPs.
		List<DiscoveryInformation> discoveries;
		try {
			discoveries = consumerManager.discover(OpSettings.GOOGLE
					.getDiscoveryUrl());
		...
		session.setAttribute(OP_ENDPOINT, discoveries.get(0).getOPEndpoint()
				.toString());
	...
	private String getUiExtensionParameters(String consumer) {
		return new StringBuilder().append("'openid.ui.icon' : 'true'")
				.toString();
	}
	...
	private static enum OpSettings {
		...
		private OpSettings(String discoveryUrl, String friendlyName,
				String image) {
