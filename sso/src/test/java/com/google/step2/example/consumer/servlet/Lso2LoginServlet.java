...
public class Lso2LoginServlet extends InjectableServlet {
	...
	private void handleDiscovery(HttpServletRequest req,
		...
		if (YES_STRING.equals(req.getParameter("usePost"))) {
			...
			handleDiscoverySucceeded_RedirectThroughGet(resp,
	...
	private void handleDiscoveryFailed(HttpServletResponse resp)
			throws IOException {
		Map<String, String> response = Maps.newHashMap();
	...
	private void handlePasswordLogin(HttpServletRequest req,
		...
		resp.getWriter().printf(
				"<h2>you logged in with username %s and password %s</h2>",
